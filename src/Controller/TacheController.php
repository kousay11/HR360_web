...
final class TacheController extends AbstractController
{
    ...
    public function index(Request $request, ?Projet $projet, TacheRepository $tacheRepository, TrelloApiService $trello_api_service, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($projet) {
            if ($query) {
                $taches = $tacheRepository->searchByProject($projet, $query);
            } elseif ($status) {
                $taches = $tacheRepository->findByStatusAndProject($projet, $status);
            } else {
                $taches = $tacheRepository->findBy(['projet' => $projet]);
            }
        } else {
            if ($query) {
                $taches = $tacheRepository->search($query);
            } elseif ($status) {
                $taches = $tacheRepository->findBy(['statut' => $status]);
            } else {
    ...
    #[Route('/filter/project/{id}', name: 'app_tache_filter_project')]
    #[Route('/filter', name: 'app_tache_filter')]
    public function filter(Request $request, ?Projet $projet, TacheRepository $tacheRepository): Response
    {
        $status = $request->query->get('status');

        if ($status === "") {
            $taches =$taches = $tacheRepository->findBy(['projet' => $projet]);
        }
        else{
        $taches = $projet
            ? $tacheRepository->findByStatusAndProject($projet, $status)
            : $tacheRepository->findBy(['statut' => $status]);
        }
        if ($request->isXmlHttpRequest()) {
            return $this->render('tache/_task_list.html.twig', [
                'taches' => $taches,
